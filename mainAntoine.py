...
def main (data,m,Niter) :
    ...
    R=0*x
    Dtau = 1/m
    b1= np.random.invgamma(4,1)
    b=[]
    for k in range (1,N+1):   #initialisation de b
        b.append(b1^k)
    ...
    for l in range (Niter):
        for j in range (n-2):
            for k in range (m-1):
                for i in range (N-1):
                    ...
                    muR= R[m*j+k-1][i] +Dtau*(-R[m*j+k-1][i])/(j+1-j+(k-1)/m)
                    sigmaR= (j+1-j+k/m)/(j+1-j+(k-1)/m)*Dtau*theta[l]^2*x[m*j+k-1][i]
        ...
        b[l+1]= np.random.invgamma(4+n*m/2,1+beta)
